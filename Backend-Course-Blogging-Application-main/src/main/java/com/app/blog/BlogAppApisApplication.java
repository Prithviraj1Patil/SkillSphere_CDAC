	...
	public void run(String... args) throws Exception {
		...
		try {
			...
			if(categoryRepository.findById(1).isEmpty()) {
				...
				category.setCategoryDescription("Content regarding technology");
				categoryRepository.save(category);
			}
			if(categoryRepository.findById(2).isEmpty()) {
				category=new Category();
				category.setCategoryTitle("Interview");
				category.setCategoryDescription("Content regarding Interview");
				categoryRepository.save(category);
			}
